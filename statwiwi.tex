...
In Kapitel \ref{chap:linreg} haben wir schon die lineare Einfachregression kennengelernt, mit der man hier die Trendkomponente der Form $g(t)=\hat{y}_t=\hat{a}+\hat{b}\cdot t$ schätzen kann.\\
...
Bei Permutationen ohne Wiederholungen gibt es nur unterschiedliche Elemente, die der Reihenfolge nach angeordnet werden. Dabei gibt es auf dem ersten Platz der Reihenfolge n verschiedene Möglichkeiten diesen zu besetzen, auf dem zweiten Platz nur noch n-1, da schon eines der Elemente auf dem ersten Platz sitzt und dieses nun nicht mehr auf dem zweiten Platz sein kann. So geht es weiter bis am Ende nur noch auf dem letzten Platz eine Möglichkeit noch besteht, nämlich das letzte noch übrige Element.\\
...
\begin{itemize}
    \item [a)]Der fünffache Münzwurf ist eine diskrete Zufallsvariable.
    \item[b)]Mit Zufallsvariablen können Ergebnisse von Zufallsexperimenten beschrieben werden, die noch nicht durchgeführt wurden.
    \item[c)]Der Träger der Zufallsvariable Würfelwurf eines 24-seitigen Würfels ist T = 1,2,3,...,24
    \item[d)] Eine stetige Zufallsvariable hat abzählbar endliche Ergebnisse.
\end{itemize}
\newpage
...
\subsection{Multiple lineare Regression}\label{sec:Reg}
