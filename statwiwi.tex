...
\begin{itemize}
    ...
    \item[b)] Der Unterschied zwischen der stetigen und diskreten Gleichverteilung ist, dass die\\ stetige Gleichverteilung unendlich viele verschiedene Werte annehmen kann. \hfill $\square$
    ...
    \item[e)] Die Dichtefunktion von $X \sim U(2,6)$ verläuft auf der Höhe 0,25. \hfill $\square$
\end{itemize}
...
\begin{itemize}
    ...
    \item[b)] Bei der Exponential-, wie auch der geometrischen Verteilung geht es darum, wann das\\ gewünschte Ereignis zum ersten/nächsten Mal auftritt. \hfill $\square$
...
\begin{itemize}
    ...
    \item[b)] Je größer die Anzahl der Freiheitsgrade, desto näher kommt sie der Standardnormalverteilung. \hfill $\square$
...
\begin{itemize}
    ...
    \item[b)] Der Wert einer Verteilungsfunktion an der Stelle $x=5$ ist $P(X\leq 5)$.\hfill $\square$
    \item[c)] Die Werte der Verteilungsfunktion entsprechen kumulierten Wahrscheinlichkeiten. \hfill $\square$
...
\begin{itemize}
    ...
    \item[c)] Die Hypothese, die man nachweisen will befindet sich in der Alternativhypothese $H_1$. \hfill $\square$
...
\begin{itemize}
    ...
    \item[d)] Die Teststatistik beim Gauss-Test ist im Gegensatz zum t-Test normalverteilt. \hfill $\square$
\end{itemize}
...
\paragraph{4. Welche Aussagen über die Hypothesentests sind wahr?} \begin{itemize}
    ...
    \item[d)] Beim Chi-Quadrat-Unabhängigkeitstest wird die Unabhängigkeit zweier Merkmale\\ überprüft, indem die beobachteten Häufigkeiten mit den zu erwarteten Häufigkeiten \\(ohne Abhängigkeit) verglichen werden.\hfill $\square$
\end{itemize}   
...
\begin{itemize}
    ...
    \item[c)] In Statistik II trifft man die Annahme, dass die Fehlerterme Zufallsvariablen sind. \hfill $\square$
...
\begin{itemize}
    ...
    \item[b)] Da in der Nullhypothese angenommen wird, dass $\beta_j$ gleich 0 ist, muss man diesen\\ hypothetischen Wert bei Berechnung der Teststatistik praktisch nicht weiter beachten. \hfill $\square$
    \item[c)] Das Konfidenzintervall liegt symmetrisch um den Schätzer. \hfill $\square$
    \item[d)] Wenn $t = -4,8$ ist und $t_{n-p-1),1-\alpha} = 4,5/2$, dann kann die Nullhypothese\\$H_0: \beta_j = 0$ nicht abgelehnt werden. \hfill $\square$
